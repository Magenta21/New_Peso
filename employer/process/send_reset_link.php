...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result->num_rows > 0) {
        ...
        $sql = "UPDATE employer SET reset_token = '$token', reset_token_expiry = '$expiry_time' WHERE email = '$email'";
        $conn->query($sql);

        // Send email with reset link
        $reset_link = "http://localhost/NEW_PESO/employer/reset_password.php?token=" . urlencode($token);

        sendResetEmail($email, $reset_link);

        echo "<script>alert('A reset link has been sent to your email address.'); window.location.href='../employer_login.php';</script>";
    } else {
        echo "<script>alert('No account found with that email address.'); window.location.href='../employer_login.php';</script>";
    }
