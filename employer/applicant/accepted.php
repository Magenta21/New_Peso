...
// Fetch total count of accepted applicants
$totalQuery = "SELECT COUNT(a.id) AS total 
               FROM applied_job aj
               JOIN applicant_profile a ON aj.applicant_id = a.id
               WHERE aj.job_posting_id = ? AND aj.status = 'Accepted'";
...
<div class="container mt-3">
    <?php
    // Fetch accepted applicants with pagination
    ...
    ?>
            <div class="row job-row border rounded mb-3 shadow-sm p-3">
                <div class="col-md-8">
                    ...
                    <p><i class="bi bi-calendar-check"></i> Accepted: <?= date('M d, Y', strtotime($applicant['application_date'])) ?></p>
                </div>
        ...
        <?php endwhile; ?>
    <?php else: ?>
        <div class="alert alert-info">No accepted applicants yet.</div>
    <?php endif; ?>
</div>
...
<div class="pagination justify-content-center mt-3">
    <?php if ($page > 1): ?>
        <a href="?id=<?= $_GET['id'] ?>&tab=accepted&page=<?= $page - 1 ?>" class="btn btn-outline-primary">Previous</a>
    <?php endif; ?>
    <span class="mx-2">Page <?= $page ?> of <?= $totalPages ?></span>
    <?php if ($page < $totalPages): ?>
        <a href="?id=<?= $_GET['id'] ?>&tab=accepted&page=<?= $page + 1 ?>" class="btn btn-outline-primary">Next</a>
    <?php endif; ?>
</div>