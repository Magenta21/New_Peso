...
// Fetch total count of applicants
...
<div class="container mt-3">
    ...
    ?>
            <div class="row job-row border rounded mb-3 shadow-sm p-3">
                <div class="col-md-8">
                    <p class="mb-1"><i class="bi bi-person"></i> <?= htmlspecialchars($applicant['fname'] . ' ' . $applicant['lname']) ?></p>
                    <p class="mb-1"><i class="bi bi-envelope"></i> <?= htmlspecialchars($applicant['email']) ?></p>
                    <p class="mb-1"><i class="bi bi-telephone"></i> <?= htmlspecialchars($applicant['contact_no']) ?></p>
                    <p class="mb-0"><i class="bi bi-calendar"></i> Applied: <?= date('M d, Y', strtotime($applicant['application_date'])) ?></p>
                </div>
                <div class="col-md-4 text-end align-self-center">
                    <div class="d-flex flex-wrap justify-content-end gap-2">
                        ...
                        <a href="update_status.php?applicant=<?= $applicant['id'] ?>&job=<?= $jobId ?>&status=Accepted" class="btn btn-sm btn-success">
                            <i class="bi bi-check-circle"></i> Accept
                        </a>
                        <a href="update_status.php?applicant=<?= $applicant['id'] ?>&job=<?= $jobId ?>&status=Interview" class="btn btn-sm btn-warning">
...
<div class="pagination justify-content-center mt-3">
    <?php if ($page > 1): ?>
        <a href="?id=<?= $_GET['id'] ?>&page=<?= $page - 1 ?>" class="btn btn-outline-primary">Previous</a>
    <?php endif; ?>
    <span class="mx-2">Page <?= $page ?> of <?= $totalPages ?></span>
    <?php if ($page < $totalPages): ?>
        <a href="?id=<?= $_GET['id'] ?>&page=<?= $page + 1 ?>" class="btn btn-outline-primary">Next</a>
    <?php endif; ?>
</div>