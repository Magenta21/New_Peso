...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "SELECT id FROM admin_profile WHERE email = '$email'";
    ...
    if ($result->num_rows > 0) {
        ...
        $sql = "UPDATE admin_profile SET reset_token = '$token', reset_token_expiry = '$expiry_time' WHERE email = '$email'";
        $conn->query($sql);

        // Send email with reset link
        $reset_link = "http://peso.rf.gd/NEW_PESO/admin/reset_password.php?token=" . urlencode($token);

        sendResetEmail($email, $reset_link);

        echo "<script>alert('A reset link has been sent to your email address.'); window.location.href='../admin_login.php';</script>";
    } else {
        echo "<script>alert('No account found with that email address.'); window.location.href='../admin_login.php';</script>";
    }
