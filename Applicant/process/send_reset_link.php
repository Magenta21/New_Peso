...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "SELECT id FROM applicant_profile WHERE email = '$email'";
    ...
    if ($result->num_rows > 0) {
        ...
        $reset_link = "http://localhost/NEW_PESO/applicant/reset_password.php?token=" . urlencode($token);

        sendResetEmail($email, $reset_link);

        echo "<script>alert('A reset link has been sent to your email address.'); window.location.href='../applicant_login.php';</script>";
    } else {
        echo "<script>alert('No account found with that email address.'); window.location.href='../applicant_login.php';</script>";
    }
